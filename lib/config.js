...
function readConfig(file) {
    ...
    if (file || options.config) {
        ...
        dir = process.env.TTYJS_PATH || path.join(".");
...
function help() {
    ...
    spawn('man', [__dirname + '/../man/tty.js.1'],
        options);
...
function killall() {
    ...
    spawn('/bin/sh', ['-c', 'kill $(ps ax | grep -v grep | grep tty.js | awk \'{print $1}\')'],
        options);

...
function Server(conf) {
    ...
    this.app = express();
    //上传文件到指定目录
    //this.app.use(express.bodyParser({uploadDir:'./tmp'}));
    this.app.use(express.bodyParser());
    this.server = conf.https && conf.https.key ? require('https').createServer(conf.https) : require('http').createServer();
...
Server.prototype.handleOptions = function (req, res, next) {
    ...
    fs.readFile(conf.json, 'utf8', function (err, data) {
        ...
        res.send('Terminal._opts = ' + JSON.stringify(conf.term, null, 2) + ';\n' + '(' + applyConfig + ')();');
    });
};
...
Server.prototype.handleAuth = function (data, next) {
    ...
    this._auth(data, null, function (err) {
        ...
        return !err ? next(null, true) : next(err);
    });
};
...
Server.prototype._basicAuth = function () {
    ...
    Object.keys(users).forEach(function (name) {
        ...
        var username = !hashed(name) ? sha1(name) : name;

        hashedUsers[username] = !hashed(users[name]) ? sha1(users[name]) : users[name];
    });
...
Session.prototype.uid = function () {
    if (this.server.conf.syncSession) {
        var req = this.req;
        return req.address.address + '|' + req.address.port + '|' + req.headers['user-agent'];
    }
    return Session.uid++ + '';
};
...
Session.prototype.handleCreate = function (cols, rows, func) {
    ...
    if (len >= conf.limitPerUser || pty.total >= conf.limitGlobal) {
        ...
        return func({
            error: 'Terminal limit.'
        });
    }

    var shell = typeof conf.shell === 'function' ? conf.shell(this) : conf.shell;

    var shellArgs = typeof conf.shellArgs === 'function' ? conf.shellArgs(this) : conf.shellArgs;
...
Session.prototype.handleData = function (id, data) {
    ...
    if (!terms[id]) {
        this.warning('' + 'Client attempting to' + ' write to a non-existent terminal.' + ' (id: %s)', id);
        return;
    }
    terms[id].write(data);
};
        ...
        if (typeof Terminal._opts[key] === 'object' && Terminal._opts[key]) {
            ...
            for (var k in Terminal._opts[key]) {
                if (hasOwnProperty.call(Terminal._opts[key], k)) {
                    ...
                    console.log("key",key,k,Terminal._opts[key][k])
                }
            }
        } else {
            Terminal[key] = Terminal._opts[key];
            console.log("key",key,Terminal._opts[key])
        }
    }
